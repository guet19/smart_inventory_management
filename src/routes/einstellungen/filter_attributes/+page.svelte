<script>
  ...
  $effect(() => {
      ...
      if (form?.success) {
          ...
          showSuccessMessage(form?.message || "Aktion erfolgreich durchgeführt!");
      }
  });
      ...
      <div class="list-group shadow-sm">
        {#each data?.attributeLibrary ?? [] as attr}
          <div class="list-group-item list-group-item-action p-0 overflow-hidden">
            <button class="w-100 border-0 bg-transparent p-3 d-flex justify-content-between align-items-center text-start" onclick={() => toggleAttribute(attr._id)}>
              <div>
            ...
            {#if expandedId === attr._id}
              <div class="p-3 bg-light border-top animate-fade-in">
                    ...
                    <div class="col-md-8">
                      ...
                      <div class="d-flex flex-wrap gap-1">
                        {#each attr.options as option}
                          <span class="badge bg-white text-primary border border-primary-subtle">{option}</span>
                        {/each}
                      </div>
                    </div>
                  {/if}
                ...
                <div class="mt-3 pt-3 border-top d-flex gap-2">
                  <button class="btn btn-sm btn-outline-secondary" onclick={() => startEdit(attr)}>Bearbeiten</button>

                  <form method="POST" action="?/delete" use:enhance onsubmit={() => confirm("Möchtest du dieses Attribut wirklich unwiderruflich löschen?")}>
