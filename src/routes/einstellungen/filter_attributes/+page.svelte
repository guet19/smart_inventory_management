<script>
    ...
    let sortedAttributes = $derived(
        [...(data.attributeLibrary || [])].sort((a, b) => a.label.localeCompare(b.label, 'de'))
    );
    ...
    let filteredAttributes = $derived(
        searchQuery.trim() === "" 
            ...
            : sortedAttributes.filter(attr => {
                ...
                const optionsMatch = attr.options && attr.options.some(opt => opt.toLowerCase().includes(query));
                return labelMatch || optionsMatch;
            })
    );

    let activeAttribute = $derived(data.attributeLibrary?.find(a => a._id === activeAttributeId));
    let filteredEditOptions = $derived(
        optionSearchQuery.trim() === "" ? editOptionsList : editOptionsList.filter(opt => opt.toLowerCase().includes(optionSearchQuery.toLowerCase().trim()))
    );
    ...
    function addEditOption() {
        if (editCurrentOption.trim() && !editOptionsList.includes(editCurrentOption.trim())) {
            ...
            <div class="col-12 animate-fade-in">
                <div class="card shadow border-0 overflow-hidden">
                    <div class="card-header bg-dark text-white p-4">
                        <span class="text-secondary small text-uppercase fw-bold">Filterattribut bearbeiten</span>
                        ...
                        <form method="POST" action="?/update" use:enhance={() => {
                            ...
                            <div class="row mb-4">
                                <div class="col-md-4 mb-3">
                                    <label for="editLabel" class="form-label small fw-bold">Anzeigename</label>
                                    <input type="text" id="editLabel" name="label" class="form-control" bind:value={editLabel} required>
                                </div>
                                <div class="col-md-4 mb-3">
                                    <label for="editUiType" class="form-label small fw-bold">Eingabetyp</label>
                                    <select id="editUiType" name="ui_type" class="form-select" bind:value={editUiType}>
                                ...
                                <div class="col-md-4 mb-3">
                                    <label for="editUnit" class="form-label small fw-bold">Einheit</label>
                                    <input type="text" id="editUnit" name="unit" class="form-control" bind:value={editUnit}>
                                </div>
                            </div>
                            ...
                            {#if editUiType === 'select'}
                                <div class="bg-white border rounded shadow-sm p-4 mb-4">
                                    <div class="d-flex justify-content-between align-items-center mb-3 pb-2 border-bottom flex-wrap gap-3">
                                        <h5 class="h6 text-muted mb-0 fw-bold">Werte & Optionen verwalten</h5>
                                        <div class="form-check form-switch mb-0">
                                            <input type="hidden" name="is_multiple" value={editIsMultiple}>
                                            <input class="form-check-input" type="checkbox" id="editIsMultipleSwitch" bind:checked={editIsMultiple}>
                                            <label class="form-check-label text-dark small" for="editIsMultipleSwitch">Mehrfachauswahl</label>
                                        </div>
                                    </div>

                                    <div class="row align-items-center mb-4">
                                        <div class="col-md-6 mb-2 mb-md-0">
                                            <label class="form-label small text-muted">Suchen (Filtert die Liste unten)</label>
                                            <div class="input-group input-group-sm shadow-sm">
                                                <span class="input-group-text bg-light"><i class="bi bi-search"></i></span>
                                                <input type="text" class="form-control" placeholder="Option suchen..." bind:value={optionSearchQuery}>
                                                {#if optionSearchQuery !== ""}
                                                    <button class="btn btn-outline-secondary bg-white" type="button" aria-label="Zurücksetzen" onclick={() => optionSearchQuery = ""}><i class="bi bi-x-lg"></i></button>
                                                {/if}
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <label class="form-label small text-muted">Neuen Wert hinzufügen</label>
                                            <div class="input-group input-group-sm shadow-sm">
                                                <input type="text" class="form-control" placeholder="z.B. M5" bind:value={editCurrentOption} onkeydown={(e) => e.key === 'Enter' && (e.preventDefault(), addEditOption())}>
                                                <button type="button" class="btn btn-primary flex-shrink-0 px-3" aria-label="Hinzufügen" onclick={addEditOption}><i class="bi bi-plus-lg">Hinzufügen</i></button>
                                            </div>
                                        </div>
                                    </div>
                                    
                                    <div class="d-flex flex-wrap gap-2 p-3 bg-light rounded border" style="min-height: 100px; max-height: 300px; overflow-y: auto;">
                                        {#each filteredEditOptions as opt}
                                            <span class="badge bg-white text-dark border border-secondary-subtle fs-6 d-flex align-items-center gap-2 shadow-sm py-2 px-3">
                                                {opt}
                                                <button type="button" class="btn-close" style="font-size: 0.6rem" aria-label="Entfernen" onclick={() => removeEditOption(opt)}></button>
                                            </span>
                                        {:else}
                                            <div class="text-muted small w-100 text-center mt-3">Keine Werte hinterlegt.</div>
                                        {/each}
                                    </div>
                            ...
                            <div class="d-flex justify-content-end gap-3 mt-4 pt-3 border-top">
                                <button type="button" class="btn btn-outline-secondary px-4" onclick={closeEditMode}>Abbrechen</button>
            ...
            <div class="col-md-4 mb-4 animate-fade-in">
                <div class="card shadow-sm border-0 bg-dark text-white p-4 sticky-top dark-scrollbar" style="top: 2rem; max-height: 85vh; overflow-y: auto;">
                    ...
                    <form method="POST" action="?/create" use:enhance={() => {
                        ...
                        {#if ui_type === 'select'}
                            <div class="mb-4 p-3 bg-secondary rounded border border-dark">
                                <div class="form-check form-switch">
                                    ...
                                    <label class="form-check-label text-white" for="isMultipleSwitch">Mehrfachauswahl erlauben</label>
                                </div>
                            </div>
                            <div class="mb-3">
                                <label for="optionInput" class="form-label small text-muted">Auswahl-Optionen</label>
                                <div class="input-group mb-2">
                                    <input type="text" id="optionInput" class="form-control bg-secondary text-white border-0" placeholder="z.B. M3" bind:value={currentOption} onkeydown={(e) => e.key === 'Enter' && (e.preventDefault(), addOption())}>
                                    <button type="button" class="btn btn-primary flex-shrink-0 px-3" aria-label="Hinzufügen" onclick={addOption}><i class="bi bi-plus-lg"></i></button>
                                </div>
                                <div class="d-flex flex-wrap gap-2 mt-2">
                                    {#each optionsList as opt}
                                        <span class="badge bg-primary d-flex align-items-center gap-2">
                                            {opt}
                                            <button type="button" class="btn-close btn-close-white" style="font-size: 0.5rem" aria-label="Entfernen" onclick={() => removeOption(opt)}></button>
                                        </span>
                                    {/each}
                                </div>
                        ...
                        {/if}
                        <button type="submit" class="btn btn-success w-100 mt-3 fw-bold">Speichern</button>
                    </form>
                </div>
            </div>
                        ...
                        <div class="list-group-item list-group-item-action p-0 overflow-hidden bg-white">
                            ...
                            >
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    {#if attr.unit}
                                        <span class="badge bg-light text-secondary border">Einheit: {attr.unit}</span>
                                    {/if}
                                    <span class="badge {attr.ui_type === 'select' ? 'bg-info text-dark' : 'bg-secondary'} rounded-pill ms-2">
                                        {attr.ui_type === 'select' ? (attr.is_multiple ? 'Multi-Dropdown' : 'Dropdown') : attr.ui_type}
                                    </span>
                                </div>
                                <i class="bi {expandedId === attr._id || searchQuery.trim() !== '' ? 'bi-chevron-up' : 'bi-chevron-down'} text-dark"></i>
                            </button>
                                ...
                                <div class="p-3 bg-light border-top animate-fade-in">
                                    ...
                                    {#if attr.ui_type === 'select'}
                                        ...
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            <h6 class="text-muted small fw-bold mb-0 text-uppercase">Hinterlegte Optionen</h6>
                                            <div class="input-group input-group-sm shadow-sm" style="width: 250px;">
                                                <span class="input-group-text bg-white"><i class="bi bi-search"></i></span>
                                                <input type="text" class="form-control" placeholder="{attr.label} durchsuchen..." bind:value={quickOptionSearchQuery}>
                                            </div>
                                        </div>
                                        ...
                                        <div class="d-flex flex-wrap gap-2">
                                            {#each filteredQuickOptions as option}
                                                <form method="POST" action="?/removeOptionQuick" use:enhance={() => {
                                                    return async ({ update }) => {
                                                        await update();
                                                    ...
                                                    <div class="badge bg-white text-dark border border-secondary-subtle px-3 py-2 shadow-sm d-flex align-items-center gap-2">
                                                        {option}
                                            ...
                                            {:else}
                                                <p class="text-muted small fst-italic mb-0">Keine passenden Werte gefunden.</p>
                                            {/each}
                                        </div>
                                    {:else}
                                    ...
                                    <div class="mt-4 pt-3 border-top d-flex justify-content-between align-items-center flex-wrap gap-3">
                                        ...
                                        {#if attr.ui_type === 'select'}
                                            ...
                                            <form method="POST" action="?/addOptionQuick" use:enhance={() => {
                                                return async ({ result, update }) => {
                                                    await update();
                                                    if (result.type === 'success') {
                                                ...
                                                <input type="hidden" name="id" value={attr._id}>
                                                <div class="input-group input-group-sm shadow-sm">
                                                    <input type="text" name="newOption" class="form-control" placeholder="Neuen Wert hinzufügen..." bind:value={quickNewOption} required>
                                                    <button type="submit" class="btn btn-success fw-bold px-3"><i class="bi bi-plus-lg"> &#10004</i></button>
                                                </div>
                                            </form>
                                        {:else}
                                            <div></div> <!-- Leerer Platzhalter für Flexbox -->
                                        {/if}

                                        <!-- HAUPTAKTIONEN UNTEN RECHTS -->
                                        <div class="d-flex gap-2">
                                            <button class="btn btn-sm btn-outline-primary fw-bold px-3 shadow-sm" onclick={() => openEditMode(attr)}>
                                                <i class="bi bi-pencil me-1"></i> Alles bearbeiten
                                            </button>
                                            <button class="btn btn-sm btn-outline-danger fw-bold px-3 shadow-sm"><i class="bi bi-trash me-1"></i> Löschen</button>
                                        </div>
                                    </div>
